...
SqlDbType.Int, 4);
   ...
   SqlDbType.NVarChar, 255);
            ...
            if (model.level_IsEdit)
            {
                strSql.Append(" level = @level,");
                parameter = new SqlParameter("@level",

   SqlDbType.Int, 4);
                parameter.Value = model.level;
                parameters.Add(parameter);
                count++;
            }
            ...
            if (model.upid_IsEdit)
            {
                strSql.Append(" upid = @upid,");
                parameter = new SqlParameter("@upid",

   SqlDbType.Int, 4);
                parameter.Value = model.upid;
                parameters.Add(parameter);
                count++;
            }
		...
		/// 得到一个对象实体
		/// </summary>
        public Model.pre_common_district DataRowToModel(DataRow row)
		{
			Model.pre_common_district model=new Model.pre_common_district();
			if (row != null)
			{
