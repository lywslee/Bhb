...
namespace DBUtility
{
    public abstract class DbHelperSQL
    {
        ...
        public static int ExecuteNonQuery(List<KeyValuePair<string, SqlParameter[]>> listkv)
        {
            using (SqlConnection conn = new SqlConnection(DbHelperSQL.connectionString))
            {
                using (SqlCommand com = new SqlCommand())
                {
                    ...
                    try
                    {
                        foreach (KeyValuePair<string, SqlParameter[]> kv in listkv)
                        {
                            if (kv.Key != null && kv.Key != string.Empty)
                            {
                                com.Parameters.Clear();
                                com.CommandText = kv.Key;
                                if (kv.Value != null)
                                {
                                    com.Parameters.AddRange(kv.Value);
                                }
                                com.ExecuteNonQuery();
                            }
                        }
