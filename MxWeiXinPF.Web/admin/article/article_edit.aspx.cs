...
namespace Web.admin.article
{
    public partial class article_edit : Web.UI.ManagePage
    {
     ...
     //   protected int channel_id;
        ...
        //{
        //    List<Model.article_attribute_field> ls = new BLL.article_attribute_field().GetModelList("is_sys=0");
        //    if (ls.Count > 0)
        //    {
        ...
        //#region 显示默认扩展字段=========================
        //private void ShowSysField()
        //{
        //    //查找该频道所选的默认字段
        //    List<Model.article_attribute_field> ls = new BLL.article_attribute_field().GetModelList("is_sys=1");
        //    foreach (Model.article_attribute_field modelt in ls)
        //    {
        //        //查找相应的控件，如找到则显示
        ...
        private void ShowInfo(int _id)
        {
            ...
            //扩展字段赋值
            //List<Model.article_attribute_field> ls1 = new BLL.article_attribute_field().GetModelList( "");
          ...
          //  model.channel_id = 0;
           ...
           // model.fields = SetFieldValues(); //扩展字段赋值
            ...
            if (bll.Update(model))
            {
                ...
                AddAdminLog(MXEnums.ActionEnum.Edit.ToString(), "修改article_list频道内容:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == MXEnums.ActionEnum.Edit.ToString()) //修改
            {
                ...
                JscriptMsg("修改信息成功！", "article_list.aspx", "Success");
            }
            else //添加
            {
                ChkAdminLevel("article_list", MXEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("添加信息成功！", "article_list.aspx", "Success");
            }
        }
    }
}