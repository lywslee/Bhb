<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Attach.aspx.cs" Inherits="Web.admin.page.ajax.Attach" %>
...
<script type="text/javascript">
    ...
    //页面加载完成执行
    $(function () {
        $(".upload-attach").InitSWFUpload({ filesize: "102400", sendurl: "/tools/upload_ajax.ashx", flashurl: "/scripts/swfupload/swfupload.swf", filetypes: "*.<%=siteConfig.fileextension.Replace(",",";*.") %>;*.mp4" });
    ...
    function execAttachHtml() {
        if ($("input[name='attachType']:checked").val() == 0) {
            if ($("#hidFilePath").val() == "" || $("#txtFileName").val() == "") {
...
//            + '<input name="hid_attach_id" type="hidden" value="0" />'
            ...
            + '<div class="info">类型：<span class="ext">' + fileExt + '</span> 下载：<span class="down">0</span>次</div>'
            + '</li>';
            $("#showAttachList", api.opener.document).children("ul").html(liHtml);
        }
        api.close();
    }
</script>
